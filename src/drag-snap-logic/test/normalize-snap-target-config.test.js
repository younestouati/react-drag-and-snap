...
test(
    'SnapTargetConfigBuilder returns a config with the default snap descriptors when no custom config is provided',
    () => {
        const config = normalizeSnapTargetConfig();
...
test(
    `SnapTargetConfigBuilder lets snapCriteria override releaseCriteria 
    and dragCriteria if these are not explicitly set`,
    () => {
...
test(
    `SnapTargetConfigBuilder lets snapTransform override releaseSnapTransform 
    and dragSnapTransform if these are not explicitly set`,
    () => {
        const snapTransform = () => {};
