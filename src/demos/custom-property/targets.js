...
import {defaultSnapping, noSnapping, withCustomSnapProps} from '../../lib/defaults/default-snap-transformers';
...
const squareConfigDefault = {
    ...
    snapTransform: withCustomSnapProps(defaultSnapping, {gray: 1})
};

const roundConfigDefault = {
    snapCriteria: Criteria.isCenterWithinRadius(100),
    snapTransform: withCustomSnapProps(defaultSnapping, {radius: 50})
};

const squareConfigProportional = {
    snapCriteria: Criteria.isCenterWithinRadius(200),
    snapTransform: withCustomSnapProps(noSnapping, ({distance}) => ({gray: 1 - distance/200}))
};

const roundConfigProportional = {
    snapCriteria: Criteria.isCenterWithinRadius(200),
    snapTransform: withCustomSnapProps(noSnapping, ({distance}) => ({radius: 50 * (1  - distance/200)}))
};
